...
if __name__ == '__main__':
    img_id, source_path, dst_path = sys.argv[1:4]
    img_paths, label_paths = crop_and_gen(img_id, source_path, dst_path)
    try:
        if sys.argv[4] == 'bbox':
            combins = list(zip(img_paths, label_paths))
            for i in range(10):
                draw_box(*combins[i], os.getcwd())
    except:
        pass
